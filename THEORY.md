...
Validate inputs before start to working in any other logic make the code clean and don't waste resources working with invalid data
...
Store data in a database ensure that the data never losses in spite of the application restarts. Data and application lives in separate servers.
...
Each request should have a fresh instances of the service and ensures that prior logic will affect new requests.
...
Having a separate component with the right format for currency help to test it, keep this logic in one place and reuse it in any other place
...
With many unit/integration test we ensure that the main logic is developed in the right way. The cost of it is lower than having many E2E tests.
With that in mind, we need only few E2E tests to just verify the whole flow in the application.
